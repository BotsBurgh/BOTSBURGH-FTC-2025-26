...
class teleOpMain : OpMode() {
    ...
    override fun loop() {
        ...
        if (Limelight.seesTag) {
            telemetry.addData("ANGLE (tx)", "%.2f deg".format(Limelight.angleX))
        } else {
            ...
            Turret.setTargetPos(
                RobotTracker.readPositionFile()[3],
                RobotTracker.readPositionFile()[4]
            )
            ...
            val currentAngle =
                Turret.aimer.currentPosition.toDouble() / RobotConfig.Turret.TICKS_PER_DEGREE
